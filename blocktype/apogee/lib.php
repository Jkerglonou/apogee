...
class PluginBlocktypeApogee extends PluginBlocktype {

    public static function get_title() {
        return get_string('title', 'blocktype.apogee/apogee');
    }

    public static function get_description() {
        return get_string('description', 'blocktype.apogee/apogee');
    }
	...
	//copie sur tasks
       $smarty = smarty_core();
        if (isset($configdata['artefactids'])) {
			...
			$template = 'artefact:apogee:vetrows.tpl';
            ...
            ArtefactTypeApogee::render_vets($les_vets, $template, $configdata, $pagination);

            if ($exporter && $vets['count'] > $vets['limit']) {
                $artefacturl = get_config('wwwroot') . 'view/artefact.php?artefact=' . $configdata['artefactids']
                    . '&amp;view=' . $instance->get('view');
                $les_vets['pagination'] = '<a href="' . $artefacturl . '">' . get_string('allvets', 'artefact.apogee') . '</a>';
            }
            $smarty->assign_by_ref('vets',$les_vets);
        }
        else {

            $smarty->assign('novets','blocktype.apogee/apogee');
        }

        $smarty->assign('blockid', $instance->get('id'));
        return $smarty->fetch('blocktype:apogee:content2.tpl');
    ...
    public static function artefactchooser_element($default=null) {
        ...
        return array(
            ...
            'title' => get_string('vetstoshow', 'blocktype.apogee/apogee'),
            'defaultvalue' => $default,
            'blocktype' => 'apogee',
            'selectone' => false,
            'search'    => false,
            'artefacttypes' => PluginArtefactApogee::get_artefact_types(),
            'template'  => 'artefact:apogee:artefactchooser-element.tpl',
        );
    }
